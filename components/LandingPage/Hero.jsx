...
import {
    Grid, Typography, Link as MuiLink, Container
} from '@mui/material';
...
const Hero = () => {
    ...
    useEffect(() => {
        const loop = setInterval(() => {
            if (sizes.filter(item => item.width && item.height).length !== images.length) {
            ...
            <Grid container alignItems='center'>
                <Grid ref={myRef} item xs={12} md={7} lg={6} pl={[5, 8, 15, 21]} pr={[5, 8]}>
                    <Typography variant='h2' component='h1' sx={{
                        ...
                        The{' '}
                        <Typography variant='h2' component='span' color='primary' sx={{
                            fontWeight: Fonts.FW_500,
                            fontSize: 'clamp(2.625rem, 1.2857rem + 3.5714vw, 4rem)'
                        }}>
                            word learning tool
                    ...
                    <Typography variant='body1' component='p' sx={{ mt: 2 }} color='text.disabled'>
                        ...
                        <b style={{ color: Colors.LOGO_RED }}>phrases</b>.
                ...
                >
                    {
                        images?.[0] && imageGridSizes?.[0] > 0 && sizes?.[0]?.width > 0 && sizes?.[0]?.height > 0 &&
                        <div style={{
