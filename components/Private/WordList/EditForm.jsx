...
import ListArrayInputs from "@components/WordForm/FormComponents/ListArrayInputs";
import ListInputs from "@components/WordForm/FormComponents/ListInputs";

import { IMAGE_ALT, VIP_TYPES, SHOW_SNACKBAR, VOCAB_TYPES, FORCE_RELOAD } from "@consts";
...
function EditForm({ open = false, setOpen, word, setCurrentWord }) {
    ...
    const [vipField, setVipField] = useState('');
    ...
    useEffect(() => {
        const loop = setInterval(() => {
            if (word?.illustration && !photo?.get("photo") && !word?.illustrationIsDefault) {
                if (photo) {
                    photo?.set("photo", word?.illustration);
                    clearInterval(loop);
                }
            }
        }, 100);
    ...
    const debounceFetchPronouce = async (e) => {
        setVipField(e.target.value);
        await debounceFunction(e);
    };
            ...
            <Box component="form" noValidate onSubmit={handleSubmit}>
                <Grid container>
                    ...
                    <Grid
                        item
                        xs={12}
                        mt={opens.secondary ? 1 : 0}
                        display={opens.secondary ? "flex" : "none"}
                    >
                        <Collapse
                            in={opens.secondary}
                            sx={{ width: "100%" }}
                    ...
                    <Divider
                        sx={{
                            width: "100%",
                            my: 1,
                            display: opens.secondary || opens.tertiary ? "flex" : "none",
                        }}
                    />
        ...
        <div>
            ...
            >
                ...
                >
                    ...
                    >
                        {!fetchingAPI ? "Update Word" : "Loading..."}
                    </Typography>
                </DialogTitle>
...
const initForm = (word) => ({
    ...
    synonyms:
        !_.isEmpty(word?.synonyms) && typeof word?.synonyms === 'string'
            ? word?.synonyms.split(",")
            : null,
    antonyms:
        !_.isEmpty(word?.antonyms) && typeof word?.antonyms === 'string'
            ? word?.antonyms.split(",")
            : null,
    clasifyVocab: word?.type2?.map(i => i.name) || [],
    public: word?.public || false,
    tags: word?.tags?.map((item) => item.name) || [],
    auto: false,
});
